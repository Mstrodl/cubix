--proc file system, mostly is just tmpfs
...
function loadFS(mountpath)
    os.debug.debug_write("procfs: loading at "..mountpath)
...
function test()
    local k = fs.open("/proc/mytmp/helpme", 'w')
    k.writeLine("help me i think i am lost")
    k.close()
    os.viewTable(fs.list("/proc/mytmp"))
end
